exec("comchat.cs");
exec("shban.cs");
if($Shifter::noSwearing) exec("SHBadwordlist.cs");
if($Shifter::ScoreTracker) exec("scoretracker.cs");
exec("remote.cs");
exec("spawn.cs");
exec("game_dmsg.cs");
exec("damage.cs");
...
if($Shifter::ObjScore == "") $Shifter::ObjScore = "True";
if($Shifter::PlrScore = "") $Shifter::PlrScore = "True";
if($Shifter::ObjScore == "False" && $Shifter::PlrScore == "False") {
    $ScoreOn = "False";
}
//================================================== Globals
if($Shifter::TeamDamage != false) $Server::TeamDamageScale = 1;
...
if($SHAntiTeamKillWarnKills == "") $SHAntiTeamKillWarnKills = 1;
if($SHAntiTeamKillBanTime == "") $SHAntiTeamKillBanTime = 600;
if($SHAntiTeamKillMaxKills == "") $SHAntiTeamKillMaxKills = 2;
if($SHAntiTeamKillProximity == "") $SHAntiTeamKillProximity = 50;
//================================================== End Globals
...
function Game::playerSpawned(%pl,%clientId,%armor) {
    ...
    }%clientId.spawntime = getsimtime();%power = IsPowerDown(gamebase::getteam(%clientId));
    echo("POWER = "@%power);
    ...
    if($Shifter::PowerCheck != "False" && !%power) {
        ...
        if(Game::playerSetSpawned(%pl,%clientId,%armor)) return true;
        return false;
    } else if(%clientId.spawntype == "standard") {
        echo("SPAWN - Normal Standard");
        standardSpawnList(%clientId);
        bottomprint(%clientId, "<jc><f2>You have spawned in <f0>"@
            $fa_armor@ "<f2> armor with a <f0>"@
            $fa_pack@ "<f2> backpack.", 10);
        if(Game::playerSetSpawned(%pl,%clientId,%armor)) return true;
        return false;
        ...
        if(%clientId.favsettings) {
            bottomprint(%clientId, "<jc><f2>You have spawned with your last bought favorites.", 10);
        } else {
            bottomprint(%clientId, "<jc><f2>You must select your favorites first before you can be spawned with them.", 10);
        }
        if(Game::playerSetSpawned(%pl,%clientId,%armor)) return true;
        return false;
    } else if($Shifter::SaveOn &&%clientId.SavedInfo &&%clientId.spawntype == "saved") {
        echo("SPAWN - Player Saved");
        savedSpawnList(%clientId);%clientId.spawntype =%clientId.spawntypetwo;
        if(Game::playerSetSpawned(%pl,%clientId,%armor)) return true;
        return false;
    }
...
function Game::playerSetSpawned(%pl,%clientId,%armor) {
        //======================================================================================== Buy Actual Items
        if($Debug) echo("Buying Inventory");%clientId.spawn = 1;%max = getNumItems();
        for(%i = 0;
            (%item = $spawnBuyList[%i,%clientId]) != "";%i++) {
...
function Game::playerSpawn(%clientId,%respawn) {
    ...
    if(%clientId.spawntype != "favs" &&%clientId.spawntype != "random" &&%clientId.spawntype != "standard" &&%clientId.spawntype != "saved") {%clientId.spawntype = $Shifter::SpawnType;
        if($debug) echo("ST2"@%clientId.spawntype@ "");
    }
    Client::clearItemShopping(%clientId);%spawnMarker = Game::pickPlayerSpawn(%clientId,%respawn);
    if(!%respawn) //================================================================================== Server Welcome Message On First Spawn
    {
        if(isFile("config\\spoonbot.cs")) {
         bottomprint(%clientId, "<jc><f0>SPOONBOT <f1>v"@
            $SPOONBOT::Version@ "\n<f0>Mission: <f1>"@
            $missionName@ "   <f0>Mission Type: <f1>"@
            $Game::missionType@ "\n<f0>Press <f1>'O'<f0> for specific objectives.", 5);
      } else {
         bottomprint(%clientId, "<jc><f0>SPOONBOT <f1>v"@
            $SPOONBOT::Version@ "\n<f0>ERROR!!<f1>\nThe file SPOONBOT.CS was not found at <f2>Tribes\\config\\spoonbot.cs<f1>\nSPOONBOT WILL NOT WORK!", 20);
      }
    ...
    if(%spawnMarker) {%clientId.guiLock = "";%clientId.dead = "";%clientId.observermode = "";
        ...
        if(!String::ICompare(Client::getGender(%clientId), "Male"))%armor = "larmor";
        else %armor = "lfemale";%pl = spawnPlayer(%armor,%spawnPos,%spawnRot);
        ...
        if(%pl != -1) {
            ...
            else {%clientId.observerMode = "pregame";
...
function Game::pickRandomSpawn(%team) {%group = nameToID("MissionGroup/Teams/team"@%team@ "/DropPoints/Random");%count = Group::objectCount(%group);
    if(!%count) return -1;%spawnIdx = floor(getRandom() * (%count - 0.1));%value =%count;
    for(%i =%spawnIdx;%i <%value;%i++) {%set = newObject("spawnset", SimSet);%obj = Group::getObject(%group,%i);
        if(containerBoxFillSet(%set, $SimPlayerObjectType | $VehicleObjectType, GameBase::getPosition(%obj), 2, 2, 4, 0) == 0) {
            deleteObject(%set);
            return %obj;
        }
        if(%i ==%count - 1) {%i = -1;%value =%spawnIdx;
        }
        deleteObject(%set);
    }
    return false;
...
function Game::pickStartSpawn(%team) {%group = nameToID("MissionGroup\\Teams\\team"@%team@ "\\DropPoints\\Start");%count = Group::objectCount(%group);
    if(!%count) return -1;%spawnIdx = $lastTeamSpawn[%team] + 1;
...
function Game::pickTeamSpawn(%team,%respawn) {
    ...
    else {%spawn = Game::pickStartSpawn(%team);
        if(%spawn == -1) return Game::pickRandomSpawn(%team);
        return %spawn;
    }
}

function Game::pickObserverSpawn(%client) {%group = nameToID("MissionGroup\\ObserverDropPoints");%count = Group::objectCount(%group);
    if(%group == -1 || !%count)%group = nameToID("MissionGroup\\Teams\\team"@
        Client::getTeam(%client)@
        "\\DropPoints\\Random");%count = Group::objectCount(%group);
    if(%group == -1 || !%count)%group = nameToID("MissionGroup\\Teams\\team0\\DropPoints\\Random");%count = Group::objectCount(%group);
    if(%group == -1 || !%count) return -1;%spawnIdx =%client.lastObserverSpawn + 1;
    if(%spawnIdx >=%count)%spawnIdx = 0;%client.lastObserverSpawn =%spawnIdx;
    return Group::getObject(%group,%spawnIdx);
}
...
function Game::notifyMatchStart(%time) {
   ...
   if(%time == 5) {
     echo("Now loading Spoonbot...");
   }
   UpdateClientTimes(%time);
}
function Game::PopupSpoonbotMessage(%text) {
   if(!isObject(Notice)) newObject(Notice, FearGui::FearGuiBox,%width - 250,%height - 157, 200, 112);
   if(!isObject(NoticeText)) newObject(NoticeText, FearGuiFormattedText, 1, 0, 190, 400);
   AddToSet(Notice, NoticeText);
   AddToSet(MainMenuGui, Notice);
   Control::SetValue(NoticeText,%text);
}

function Game::RemoveSpoonbotMessage() {%text = " ";
   Control::SetValue(NoticeText,%text);
...
function Game::startMatch() // game.cs
    {
        ...
        echo("\"M\""@
            $missionName@ "\""@ ($Server::timeLimit * 60) + $missionStartTime - getSimTime()@
            "\"");
        //======================= Create Power Object Tree
        DumpObjectTree();%numTeams = getNumTeams();
        for(%i = 0;%i <%numTeams;%i =%i + 1) {
            if($TeamEnergy[%i] != "Infinite") schedule("replenishTeamEnergy("@%i@ ");", $secTeamEnergy);
        }
        echo("Match Observer");
        for(%cl = Client::getFirst();%cl != -1;%cl = Client::getNext(%cl)) {
            if(%cl.observerMode == "pregame") {%cl.observerMode = "";
...
function Game::initialMissionDrop(%clientId) {
    ...
    if(isObject($BotHUD[0])) deleteObject($BotHUD[0]);
    if(isObject($BotHUD[1])) deleteObject($BotHUD[1]);
    if(isObject(BotHUD_Frame)) deleteObject(BotHUD_Frame);
    if(isObject(BotHUD_Main)) deleteObject(BotHUD_Main);
    if($Server::TourneyMode) {%clName = Client::getName(%clientId);
        if($debug) echo($Shifter::tag0@ " "@%clName@ " "@
            String::findSubStr(%clName, $Shifter::Tag0@%clientId));
        if($debug) echo($Shifter::tag1@ " "@%clName@ " "@
            String::findSubStr(%clName, $Shifter::Tag1@%clientId));
        ...
        } else GameBase::setTeam(%clientId, -1);
    } else {
        if(%clientId.observerMode == "observerFly" ||%clientId.observerMode == "observerOrbit") {%clientId.observerMode = "observerOrbit";%clientId.guiLock = "";
            Observer::jump(%clientId);
            return;
        }%numTeams = getNumTeams();%curTeam = Client::getTeam(%clientId);
        if(%curTeam >=%numTeams || (%curTeam == -1 && (%numTeams < 2 || $Server::AutoAssignTeams))) Game::assignClientTeam(%clientId);
    }
    Client::setControlObject(%clientId, Client::getObserverCamera(%clientId));%camSpawn = Game::pickObserverSpawn(%clientId);
    Observer::setFlyMode(%clientId, GameBase::getPosition(%camSpawn), GameBase::getRotation(%camSpawn), true, true);
    if(Client::getTeam(%clientId) == -1) {%clientId.observerMode = "pickingTeam";
        if($Server::TourneyMode && ($matchStarted || $matchStarting)) {%clientId.observerMode = "observerFly";
            return;
        ...
        if(%clientId.justConnected) {
            ...
            bottomprint(%clientId, $Server::JoinMOTD, 0);%clientId.observerMode = "justJoined";%clientId.justConnected = "";
        } else if(%clientId.observerMode == "justJoined") {
            centerprint(%clientId, "");%clientId.observerMode = "";
...
function processMenuuseCustom(%clientId,%skin) {
    ...
    if(%team != -2) {
        if($TeamEnergy[%team] != "Infinite") $TeamEnergy[%team] += $InitialPlayerEnergy;%clientId.teamEnergy = 0;
...
function processMenuPickTeam(%clientId,%team,%adminClient) // admin.cs
{
    ...
    if(%clientId.observerMode == "justJoined") {%clientId.observerMode = "";
        centerprint(%clientId, "");
    }
    if((!$matchStarted || !$Server::TourneyMode ||%adminClient) &&%team == -2) {
        if(Observer::enterObserverMode(%clientId)) {
            //TS
            echo("\"A\""@%clientId@ "\""@%team@ "\"");%clientId.notready = "";
            if(%adminClient == "") messageAll(0, Client::getName(%clientId)@
                " became an observer.");
            else messageAll(0, Client::getName(%clientId)@
                " was forced into observer mode by "@
                $AdminName@ ".");
    ...
    GameBase::setTeam(%clientId,%team);%clientId.teamEnergy = 0;
    Client::clearItemShopping(%clientId);
    if(Client::getGuiMode(%clientId) != 1) Client::setGuiMode(%clientId, 1);
    Client::setControlObject(%clientId, -1);
    Game::playerSpawn(%clientId, false);%team = Client::getTeam(%clientId);
    if($TeamEnergy[%team] != "Infinite") $TeamEnergy[%team] += $InitialPlayerEnergy;
    if($Server::TourneyMode && !$CountdownStarted) {
        cneterprint(%clientId, "<f1><jc>Press FIRE when ready.", 0);%clientId.notready = true;
    }
}

function Game::ForceTourneyMatchStart() {%playerCount = 0;
    for(%cl = Client::getFirst();%cl != -1;%cl = Client::getNext(%cl)) {
        if(%cl.observerMode == "pregame")%playerCount++;
    }
    if(%playerCount == 0) return;
    for(%cl = Client::getFirst();%cl != -1;%cl = Client::getNext(%cl)) {
        if(%cl.observerMode == "pickingTeam") processMenuInitialPickTeam(%cl, -2); // throw these guys into observer
        for(%cl = Client::getFirst();%cl != -1;%cl = Client::getNext(%cl)) {%cl.notready = "";%cl.notreadyCount = "";
            bottomprint(%cl, "", 0);
        }
    }
    Server::Countdown(30);
...
function Game::CheckTourneyMatchStart() {
   ...
   %playerCount = 0;%notReadyCount = 0;
    for(%cl = Client::getFirst();%cl != -1;%cl = Client::getNext(%cl)) {
        if(%cl.observerMode == "pickingTeam") {%notReady[%notReadyCount] =%cl;%notReadyCount++;
        } else if(%cl.observerMode == "pregame") {
            if(%cl.notready) {%notReady[%notReadyCount] =%cl;%notReadyCount++;
            } else %playerCount++;
        }
    }
    if(%notReadyCount) {
        if(%notReadyCount == 1) MessageAll(0, Client::getName(%notReady[0])@
            " is holding things up!");
        else if(%notReadyCount < 4) {
            for(%i = 0;%i <%notReadyCount - 2;%i++)%str = Client::getName(%notReady[%i])@
            ", "@%str;%str =%str@ Client::getName(%notReady[%i])@
            " and "@
            Client::getName(%notReady[%i + 1])@
...
function Game::resetScores(%client) {
    ...
    if(%client == "") {
        for(%cl = Client::getFirst();%cl != -1;%cl = Client::getNext(%cl)) {
            if($debug) echo("*** Begin Scores For "@%cl);%cl.scoreKills = 0;%cl.scoreDeaths = 0;%cl.ratio = 0;%cl.score = 0;%cl.FlagCaps = 0;
            if(%cl.TKCount > 0)%cl.TKCount =%cl.TKCount;
            else %cl.TKCount = 0;
        }
        ...
        if($Shifter::Saveon) {
            ...
            if(%client.TKCount > 0)%client.TKCount =%cl.TKCount;
            else %client.TKCount = 0;%client.BadWordsSpoken = 0;%client.scoreKills = 0;%client.scoreDeaths = 0;%client.FlagCaps = 0;%client.ratio = 0;%client.score = 0;%client.ismuted = False;%client.telepoint = False;
        }
    }
}

function remoteSetArmor(%player,%armorType) {%client = Player::getClient(%player);
    if(%client.isGod) {
        Player::setArmor(%player,%armorType);
    }
}
//========================================================================================================== SetUp Players Stats On Connect
function Game::onPlayerConnected(%playerId) {%playerId.scoreKills = 0;%playerId.scoreDeaths = 0;%playerId.score = 0;%playerId.TKCount = 0;%playerId.FlagCaps = 0;%playerId.justConnected = true;%playerId.favsettings = "False";%playerId.Plastic = 15;%playerId.boosted = 0;%playerId.telepoint = False;%playerId.hackattempt = 0;%clientId.booster = 0;%ClientId.boostercool = 0;
    for(%i = 0;%i <= 21;%i++) {%playerId.fav[%i] = "";
...
function Game::assignClientTeam(%clientId) // game.cs
    {
        if($teamplay) {%name = Client::getName(%clientId);%numTeams = getNumTeams();%numPlayers = getNumClients();
            for(%i = 0;%i <%numTeams;%i =%i + 1)%numTeamPlayers[%i] = 0;
            for(%i = 0;%i <%numPlayers;%i =%i + 1) {%pl = getClientByIndex(%i);
                if(%pl !=%clientId) {%team = Client::getTeam(%pl);%numTeamPlayers[%team] =%numTeamPlayers[%team] + 1;
                }
            }%leastPlayers =%numTeamPlayers[0];%leastTeam = 0;
            for(%i = 1;%i <%numTeams;%i++) {
                if((%numTeamPlayers[%i] <%leastPlayers) || (%numTeamPlayers[%i] ==%leastPlayers && $teamScore[%i] < $teamScore[%leastTeam])) {%leastTeam =%i;%leastPlayers =%numTeamPlayers;
                }
            }
...
function Game::setRandomTeam() {
    bottomprintall("<jc>Randomly Assigning Teams - Occurs Every "@
        $Shifter::TeamJuggle@ " Missions.");%numTeams = getNumTeams();%numPlayers = getNumClients();
    ...
    for(%i = 0;%i <%numPlayers;%i++) {%pl = getClientByIndex(%i);%player[%i] =%pl;
    ...
    for(%k = 0; AllOnTeam() || (%assigned <%numplayers);%k++) {
        if(%k > 2000) {
            ...
            break;
        }%rnd = floor(getRandom() * (%numPlayers));%playerId =%player[%rnd];
        if(gamebase::getteam(%playerId) == -1 && (%playerId.observerMode != "observerOrbit" &&%playerId.observerMode != "observerFly")) {
            Game::assignClientTeam(%playerId);%name = Client::getName(%playerId);
            echo("**** Player <"@%playerId@ "> "@%name@ "  Assigned To Team "@
                gamebase::getteam(%playerId));
            echo("**** Observermode "@%playerId.observermode);%assigned++;
        }
    }
    echo("***********************");
...
function AllOnTeam() {%numPlayers = getNumClients();
    for(%i = 0;%i <%numPlayers;%i++) {%pl = getClientByIndex(%i);
...
function Player::onKilled(%this) {
    echo("*** Player Killed ***");%this.Station = "";
    $killedflagcarry = "False";%killedflag = "false";%cl = GameBase::getOwnerClient(%this);%cl.dead = 1;%cl.heatup = "0";%cl.heatlock = "0";%cl.plasmacharge = "0";%cl.charging = "0";%cl.lascharge = "0";%cl.boosted = 0;
    $StimTime[%cl] = "0";%cl.ovd = 0;
    if($AutoRespawn > 0) schedule("Game::autoRespawn("@%cl@ ");", $AutoRespawn,%cl);
    if(%this.outArea == 1) leaveMissionAreaDamage(%cl);
    Player::setDamageFlash(%this, 0.75);
    for(%i = 0;%i < 8;%i =%i + 1) {%type = Player::getMountedItem(%this,%i);
        if(%type == "flag") $killedflagcarry = "True";
        if(%type != -1) {
            if(%i != $WeaponSlot || !Player::isTriggered(%this,%i) || getRandom() > "0.6") Player::dropItem(%this,%type);
        }
    }
    if(%cl != -1) {
        if(%this.vehicle != "") {
            ...
            } else {%this.vehicle.Seat[%this.vehicleSlot - 2] = "";%this.vehicleSlot = "";
            }%this.vehicle = "";
        }%kdpos = GameBase::getPosition(%this);%this.TKDeathPos =%kdpos;%cl.lastkillpos =%kdpos;
        $killedarmor = Player::getArmor(%this);
        schedule("GameBase::startFadeOut("@%this@ ");", $CorpseTimeoutValue,%this);
        Client::setOwnedObject(%cl, -1);
        Client::setControlObject(%cl, Client::getObserverCamera(%cl));
        Observer::setOrbitObject(%cl,%this, 5, 5, 5);
        schedule("deleteObject("@%this@ ");", $CorpseTimeoutValue + 2.5,%this);%cl.observerMode = "dead";%cl.dieTime = getSimTime();
    }
}

function Client::onKilled(%playerId,%killerId,%damageType,%vertPos,%quadrant) {%score = 0;%playerId.lascharge = "";%playerId.charging = "";
    if($teamplay && (Client::getTeam(%killerId) == Client::getTeam(%playerId)) && (%killerId != playerId)) echo("\"T\""@%killerId@ "\""@%playerId@ "\""@%damageType@ "\""@
        "1"@
        "\"");
    else echo("\"K\""@%killerId@ "\""@%playerId@ "\""@%damageType@ "\""@
        "1"@
        "\"");
    echo("*** Player "@%playerId);
    echo("*** Killer "@%killerId);%killedflag = $killedflagcarry;%playerId.guiLock = true;
    Client::setGuiMode(%playerId, $GuiModePlay);
    if(!String::ICompare(Client::getGender(%playerId), "Male")) {%playerGender = "his";
    } else {%playerGender = "her";
    }%ridx = floor(getRandom() * ($numDeathMsgs - 0.01));%victimName = Client::getName(%playerId);
    ...
    {%oopsMsg = sprintf($deathMsg[-2,%ridx],%victimName,%playerGender);
        messageAll(0,%oopsMsg, $DeathMessageMask);%playerId.scoreDeaths++;
        if(%killedflag) {%playerId.score = (%playerId.score - $Score::FlagDef);
        ...
        if(!String::ICompare(Client::getGender(%killerId), "Male")) {%killerGender = "his";
        } else {%killerGender = "her";
        }
        if($teamplay && (Client::getTeam(%killerId) == Client::getTeam(%playerId))) {
            if(%damageType != $MineDamageType) messageAll(0, strcat(Client::getName(%killerId), " mows down ",%killerGender, " teammate, ",%victimName), $DeathMessageMask);
            else messageAll(0, strcat(Client::getName(%killerId), " killed ",%killerGender, " teammate, ",%victimName, " with a mine."), $DeathMessageMask);%killerId.scoreDeaths++;%killerId.score--;
            Game::refreshClientScore(%killerId);
            if($Shifter::TeamKillOn == "True") {
                CheckTeamKiller(%killerId,%playerId,%damagetype,%vertPos,%quadrant);
            }
            return;
        } else {%obitMsg = sprintf($deathMsg[%damageType,%ridx], Client::getName(%killerId),%victimName,%killerGender,%playerGender);
            messageAll(0,%obitMsg, $DeathMessageMask);
            if($Shifter::PlrScore != "False") {%score = (Scoring::killpoints(%playerId,%killerId,%damagetype,%vertPos,%quadrant));%killedpos = $lastkillpos; //== Killed Pos
               ...
               %killdist = Vector::getDistance(%killedpos,%killerpos); //== Distance from Killed Player To Enemy Flag
                if(%vertPos == "head" && (%damagetype == $SniperDamageType ||%damagetype == $BulletDamageType ||%damagetype == $LaserDamageType)) {%msg = "HEAD SHOT";
                }
                if($Shifter::JustSpawned[%playerId]) {%score = floor(%score / 2);
           ...
           %killerId.score = (%killerId.score +%score); //=== Killer Score
            if(%killedflag) {
                ...
                messageAll(0, Client::getName(%killerId)@
                    " gets a bonus for killing the flag runner!!!");
            } else {
...
function CheckTeamKiller(%killerId,%playerId,%damagetype,%vertPos,%quadrant) {
    if(%damagetype != $MineDamageType &&%damagetype != $GravDamageType &&%damagetype != $CloakDamageType) {%ppos =%playerid.TKDeathPos;
        for(%cl = Client::getFirst();%cl != -1;%cl = Client::getNext(%cl)) {
            if($teamplay && (Client::getTeam(%killerId) != Client::getTeam(%cl))) {
                if(%cl !=%playerId &&%cl !=%killerId) {%pppos = GameBase::getPosition(%cl);%dist = Vector::getDistance(%ppos,%pppos);
                    if($debug) echo("***"@
                        Client::getName(%cl)@
                        " was "@%dist@ " from "@
                        Client::getName(%playerId)@
                        " Prox = "@
                        $SHAntiTeamKillProximity);
                    if(%dist < $SHAntiTeamKillProximity) {
                        if($debug) echo("***Player "@
                            Client::getName(%playerId)@
                            " was close, kill is accidental.");
        ...
        messageAll(0, "***"@
            Client::getName(%killerId)@
            " TeamKilled***");
        $killedflagcarry = "False"; //== Reset
       %playerId.lastkillpos = -1; //== Reset
        $killedarmor = -1; //== Reset
       %score = (Scoring::killpoints(%playerId,%killerId,%vertPos,%quadrant));
        if($Shifter::JustSpawned[%playerId] == "false") {%killerId.scoreDeaths++; //=== Killer Number Of Kills
           %killerId.score = (%killerId.score -%score); //=== Killer Score
            if($Shifter::Debug) echo("*** Player Team Killed - Deducting Points");
        } else {%score = floor(%score / 2);%killerId.scoreDeaths++; //=== Killer Number Of Kills
           %killerId.score = (%killerId.score -%score); //=== Killer Score
        ...
        if(%killedflag) {
            ...
            messageAll(0, Client::getName(%killerId)@
                " TEAM KILLED his own Flag Runner!!!");
        } else {
        ...
        if(%killerid.TKCount == "") {%killerid.TKCount = 1;
        } else {%killerid.TKCount =%killerid.TKCount + 1;
            //============================================================================== Server Terminates Team Killer
            if(%killerid.TKCount > $Shifter::KillTerm) {
                if($Shifter::KillTerm > "1") {
                    bottomprintall("***"@
                        Client::getName(%killerId)@
                        " Was Terminated For TeamKilling More Than "@
                        $Shifter::KillTerm@ " Times.", 5);
                    schedule("Player::Kill("@%killerId@ ");", 2.0);
                } else {
                    bottomprintall("***"@
                        Client::getName(%killerId)@
                        " Was Terminated For TeamKilling More Than 1 Time.", 5);
                    schedule("Player::Kill("@%killerId@ ");", 2.0);
                }
            }
            if(%killerid.TKCount > $SHAntiTeamKillMaxKills) {%clientId =%killerId;%addr = Client::getTransportAddress(%killerId);%name = Client::getName(%clientId);
                ...
                bottomprintall("***"@
                    Client::getName(%killerId)@
                    " Was Kicked and Banned For TeamKilling", 5);%ip = Client::getTransportAddress(%killerId);
                BanList::add(%ip, $SHAntiTeamKillBanTime); //=== Add Player To BanList
                shban(%ip);%killer = Client::getOwnedObject(%killerId);
                messageAll(0, Client::getName(%killerId)@
                    " was kicked and banned for teamkilling");
                KickPlayer(%killerId, "***"@
                    $Shifter::TeamKillMsg@ "***");
            } else {
...
// ======================================================================================= FUBAR's Match Mode
function MatchAssign() {%numPlayers = getNumClients();
    for(%i = 0;%i <%numPlayers;%i =%i + 1) {%pl = getClientByIndex(%i);%clName = Client::getName(%pl);
        if($debug) echo($Shifter::tag0@ " "@%clName@ " "@
            String::findSubStr(%clName, $Shifter::Tag0@%pl));
        if($debug) echo($Shifter::tag1@ " "@%clName@ " "@
            String::findSubStr(%clName, $Shifter::Tag1@%pl));
        if(String::findSubStr(%clName, $Shifter::Tag0) == 0) {
            GameBase::setTeam(%pl, 0);
        } else if(String::findSubStr(%clName, $Shifter::Tag1) == 0) {
            GameBase::setTeam(%pl, 1);
        } else GameBase::setTeam(%pl, -1);
    }
    return;
}